...
{ 
  ...
  if(!is.null(df$cat_prop) && sum(df$cat_prop)!=1){
    ...
    time<-NULL
    status<-NULL
    if(is.null(df$center.size)){
      pats_grp<-rmultinom(n=1,size=N,prob=df$cat_prop)
    }else{
      if(sum(df$center.size) != N){stop("N != sum(df$m)")}
      pats_grp<-matrix(df$center.size)
    }
...
#' @export
getpower.multicenter<-function(nSim,N,duration=24,rho=1,beta,med.TimeToCensor=14,df,dist=NULL, method, type,scenario,
                        prop.fullexp=0,maxrelexptime=1,min.futime=0,min.postexp.futime=0,output.fn=NULL,simu.plot=FALSE) 
{
#  lambda<-log(2)/med.TTE.Control
  ...
  for(k in 1:nSim)
  {
    ...
    if(method != "frailty" && method != "fixed effects"){
    ...
    else if(method == "frailty"  | method == "fixed effects"){
      if(method == "frailty"){
      ...
      else if(method == "fixed effects"){
        fit <- coxph(Surv(start,stop, status) ~ factor(x)+factor(clst_id), data=dat)
        sfit <- survfit(Surv(start,stop, status) ~ factor(x)+factor(clst_id), data=dat)
        res[k,"signif"] <- ifelse(summary(fit)$coef["factor(x)1","Pr(>|z|)"]<alpha,1,0)
      }
